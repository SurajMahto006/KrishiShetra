 ...
 */
const createOrderFromInquiry = async (req, res) => {
  let session = null;
  let useTransaction = false;

  try {
    const { inquiryId, deliveryType, deliveryAddress, buyerNote } = req.body;
    ...
    // 5. Validate Delivery Inputs if delivery is requested
    const selectedDeliveryType = deliveryType === 'pickup' ? 'pickup' : 'delivery';
    ...
    // 6. Server-side price and total amount calculation (immutable transaction snapshot)
    ...
    // 8. Try initiating a MongoDB transaction session if supported by environment
    try {
    ...
    // 9. Atomically decrement lot availability to prevent race conditions / overselling
    ...
    // Mark lot sold if no available inventory remains
      ...
      [
        {
          ...
          buyerNote: buyerNote ? String(buyerNote).trim() : ''
        }
      ],
      sessionOpts
    );
...
/**
 * @desc    Update Order operational status in the delivery lifecycle
...
const updateOrderStatus = async (req, res) => {
  try {
    const { orderId } = req.params;
    const { status: newStatus, farmerNote } = req.body;
    ...
    if (!allowed.includes(newStatus)) {
      return res.status(400).json({
        success: false,
        message: `Invalid status transition from '${currentStatus}' to '${newStatus}'. Allowed next status: [${allowed.join(', ')}]`
      });
    }

    order.status = newStatus;
    if (farmerNote !== undefined) {
...
/**
 * @desc    Cancel an order before dispatch/pickup and restore available inventory
...
const cancelOrder = async (req, res) => {
  try {
    ...
    // Cancellation only permitted before picked_up
    const cancellableStatuses = ['confirmed', 'processing', 'ready_for_pickup'];
    if (!cancellableStatuses.includes(order.status)) {
      return res.status(400).json({
        success: false,
        message: `Order cannot be cancelled in '${order.status}' status (only allowed before picked_up)`
      });
    }

    order.status = 'cancelled';
    order.cancelledBy = req.user._id;
    order.cancellationReason = String(reason).trim().slice(0, 500);
    order.cancelledAt = new Date();
    await order.save();

    // Safely restore lot quantity and status
