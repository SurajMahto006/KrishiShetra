...
class _LogisticsScreenState extends ConsumerState<LogisticsScreen> {
  ...
  Future<void> _fetchAndListen() async {
    ...
    final initial = await trackingRepo.getTrackingDetails(widget.orderId);
    if (mounted) {
      setState(() {
        _trackingData = initial;
        _loading = false;
      });
    }

    trackingRepo.listenToLiveUpdates(widget.orderId, onUpdate: (updated) {
      if (mounted) {
        setState(() {
  ...
  @override
  void dispose() {
    ref.read(trackingRepositoryProvider).stopListening(widget.orderId);
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final order     = orderRepo.findById(widget.orderId) ?? orderRepo.getOrders().first;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Logistics & Live GPS'),
        leading: IconButton(
                        ...
                        child: Column(
                          ...
                          children: [
                            Row(
                              children: [
                                Text(
                                  tracking?.status ?? logistics.status,
                            ...
                            Text(
                              'ETA: ${tracking?.estimatedArrivalMinutes ?? 32} mins · Distance: ${tracking?.distanceRemainingKm.toStringAsFixed(1) ?? '18'} KM',
                    ...
                    children: [
                      ...
                      Positioned(
                        ...
                        child: Container(
                          ...
                          child: Row(
                            children: [
                              const Icon(Icons.speed, size: 14, color: AppColors.sage),
                              const SizedBox(width: 4),
                              Text(
                                '${tracking?.speed.toStringAsFixed(0) ?? '42'} km/h',
                      ...
                      Positioned(
                        ...
                        child: Container(
                          ...
                          child: Text(
                            'GPS: ${tracking?.latitude.toStringAsFixed(4) ?? '19.9975'}° N, ${tracking?.longitude.toStringAsFixed(4) ?? '73.7898'}° E',
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _routeNode(
                        ...
                        label: 'Pickup',
                        value: tracking?.pickupLocation ?? logistics.pickupAddress,
                      ),
                      // Route line
                      Padding(
                      ...
                      _routeNode(
                        ...
                        label: 'Delivery',
                        value: tracking?.dropLocation ?? logistics.deliveryAddress,
                      ),
                    ],
                  ),
                ),
...
class _MockMapPainter extends CustomPainter {
  final double driverOffsetRatio;
  _MockMapPainter({required this.driverOffsetRatio});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final path = Path()
      ...
      ..cubicTo(size.width * 0.35, size.height * 0.25, size.width * 0.65, size.height * 0.75, size.width * 0.9, size.height * 0.25);
    canvas.drawPath(path, routePaint);

    // Driver live pin
    final driverX = size.width * driverOffsetRatio;
    final driverY = size.height * 0.48;

    final pulsePaint = Paint()..color = AppColors.amber.withOpacity(0.25);
    canvas.drawCircle(Offset(driverX, driverY), 16, pulsePaint);

    final pinPaint = Paint()..color = AppColors.amber;
    canvas.drawCircle(Offset(driverX, driverY), 8, pinPaint);
  }
